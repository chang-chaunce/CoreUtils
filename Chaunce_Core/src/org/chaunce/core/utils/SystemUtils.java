package org.chaunce.core.utils;
...
import org.chaunce.core.constants.DataConstants;
...
public class SystemUtils {
	...
	public static String getApkFilePath(final Context pContext)
		...
		try {
			return packMgmr.getApplicationInfo(
					SystemUtils.getPackageName(pContext), 0).sourceDir;
	...
	private static PackageInfo getPackageInfo(final Context pContext)
			throws SystemUtilsException {
		try {
			return pContext.getPackageManager().getPackageInfo(
	...
	public static boolean hasSystemFeature(final Context pContext,
			final String pFeature) {
		try {
			final Method PackageManager_hasSystemFeatures = PackageManager.class
					.getMethod("hasSystemFeature", new Class[] { String.class });
			return (PackageManager_hasSystemFeatures == null) ? false
					: (Boolean) PackageManager_hasSystemFeatures.invoke(
							pContext.getPackageManager(), pFeature);
	...
	/**
	 * @param pBuildVersionCodeMin
	 *            taken from {@link Build.VERSION_CODES}.
	 * @param pBuildVersionCodeMax
	 *            taken from {@link Build.VERSION_CODES}.
	 */
	public static boolean isAndroidVersion(final int pBuildVersionCodeMin,
			final int pBuildVersionCodeMax) {
		return (Build.VERSION.SDK_INT >= pBuildVersionCodeMin)
				&& (Build.VERSION.SDK_INT <= pBuildVersionCodeMax);
	}

	public static float getCPUBogoMips() throws SystemUtilsException {
		final MatchResult matchResult = SystemUtils.matchSystemFile(
				"/proc/cpuinfo", SystemUtils.BOGOMIPS_PATTERN, 1000);
	...
	public static long getSystemMemorySize() throws SystemUtilsException {
		final MatchResult matchResult = SystemUtils.matchSystemFile(
				"/proc/meminfo", SystemUtils.MEMTOTAL_PATTERN, 1000);
	...
	public static long getSystemMemoryFreeSize() throws SystemUtilsException {
		final MatchResult matchResult = SystemUtils.matchSystemFile(
				"/proc/meminfo", SystemUtils.MEMFREE_PATTERN, 1000);
	...
	public static long getCPUFrequencyCurrent() throws SystemUtilsException {
		return SystemUtils
				.readSystemFileAsLong("/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq");
	}
	...
	public static long getCPUFrequencyMin() throws SystemUtilsException {
		return SystemUtils
				.readSystemFileAsLong("/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_min_freq");
	}
	...
	public static long getCPUFrequencyMax() throws SystemUtilsException {
		return SystemUtils
				.readSystemFileAsLong("/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq");
	}
	...
	public static long getCPUFrequencyMinScaling() throws SystemUtilsException {
		return SystemUtils
				.readSystemFileAsLong("/sys/devices/system/cpu/cpu0/cpufreq/scaling_min_freq");
	}
	...
	public static long getCPUFrequencyMaxScaling() throws SystemUtilsException {
		return SystemUtils
				.readSystemFileAsLong("/sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq");
	}

	private static MatchResult matchSystemFile(final String pSystemFile,
			final String pPattern, final int pHorizon)
			throws SystemUtilsException {
		InputStream in = null;
		try {
	...
	private static long readSystemFileAsLong(final String pSystemFile)
			throws SystemUtilsException {
		InputStream in = null;
		try {
			final Process process = new ProcessBuilder(new String[] {
	...
	public static long getNativeHeapAllocatedSize() {
		return Debug.getNativeHeapAllocatedSize()
				/ DataConstants.BYTES_PER_KILOBYTE;
	}
	...
	public static long getDalvikHeapAllocatedSize() {
		return SystemUtils.getDalvikHeapSize()
				- SystemUtils.getDalvikHeapFreeSize();
	}
